...
HTTP Request
     ...
     ▼
┌─────────────┐
│  Middleware  │  JWT doğrulama, CORS, günlükleme
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  Handlers   │  İstekleri ayrıştır, servisleri çağır, yanıtları biçimlendir
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  Services   │  İş mantığı (auth, oda yönetimi)
└──────┬──────┘
       │
       ▼
┌─────────────┐
│ Repository  │  GORM ile veritabanı sorguları
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  Database   │  SQLite (geliştirme) veya PostgreSQL (üretim)
└─────────────┘
```
