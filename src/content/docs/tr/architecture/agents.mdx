...
```
1. Toplantı URL'sini ayrıştır ──► Oda adını çıkar
2. Misafir girişi ──► POST /api/auth/guest-login
3. Odaya katıl ──► POST /api/room/join (LiveKit token al)
4. LiveKit'e bağlan ──► WebSocket bağlantısı
5. Kanalları yayınla ──► Ses/video karelerini akıt
```

```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│  Medya Kaynak │────►│ Python Agent │────►│   LiveKit     │
│  (dosya/URL)  │     │  (FFmpeg)    │     │   (WebRTC)    │
└──────────────┘     └──────────────┘     └──────────────┘
                            │
                     Misafir girişi + katılım
                            │
                     ┌──────────────┐
                     │ Bedrud Server│
                     └──────────────┘
