...
```
۱. تحلیل URL جلسه ──► استخراج نام اتاق
۲. ورود مهمان ──► POST /api/auth/guest-login
۳. پیوستن به اتاق ──► POST /api/room/jin (دریافت توکن LiveKit)
۴. اتصال به LiveKit ──► اتصال WebSocket
۵. انتشار ترک‌ها ──► استریم فریم‌های صوتی/ویدیویی
```
