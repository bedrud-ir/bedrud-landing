...
|---------------|---------|
| Ein Linux-Server | amd64 oder arm64, mit `sudo`-Zugriff und `systemd`. macOS/Windows: Docker oder WSL2 für Server |
...
> **Server-Einrichtung erfordert Linux mit systemd.** macOS und Windows unterstützen `bedrud install` nicht direkt — verwenden Sie [Docker](#docker) oder [WSL2](#nicht-linux-systeme).
